...
class Pager {
	...
	public function __construct($id, $pageLink = '', $useGeneralParam = false) {
		...
		$this->paramPageNumber = 'strona'.$id;
		if (is_null($pageLink))
			$this->makeLink();
		else
	...
	protected function toString() {
		...
		if (isset($this->array[self::GOTO_FIRST]))
			$_str .= $this->createHTMLLink('Pierwsza strona', $this->array[self::GOTO_FIRST][self::LINK], '<em class="fa fa-angle-double-left"></em>').$sep;
		if (isset($this->array[self::GOTO_PREV]))
			$_str .= $this->createHTMLLink('Poprzednia strona', $this->array[self::GOTO_PREV][self::LINK], '<em class="fa fa-angle-left"></em>').$sep;

		foreach ($this->array[self::PAGES] as $_page => $_pageLink){
			if (!$_pageLink)
				$_str .= '<li class="active"><a>'.$_page.'</a></li>';
			else
		...
		if (isset($this->array[self::GOTO_NEXT]))
			$_str .= $sep.$this->createHTMLLink('Następna strona', $this->array[self::GOTO_NEXT][self::LINK], '<em class="fa fa-angle-right"></em>');
		if (isset($this->array[self::GOTO_LAST]))
			$_str .= $sep.$this->createHTMLLink('Ostatnia strona', $this->array[self::GOTO_LAST][self::LINK], '<em class="fa fa-angle-double-right"></em>');
	...
	{
		$linkForm = false;
		$linkForm .= (isset($_GET['file']) && !empty($_GET['file'])) ? '&amp;file='.$_GET['file'] : false;
		return '<li><a title="'.$title.'" href="funcs.php?name='.$_GET['name'].$linkForm.$link.'">'.$text.'</a></li>';
	}
