...
class Pager extends main {
	...
	protected $actualNavPages = 0;

	/** IloĹÄ x stron */
	protected $totalNavPages = 0;
	...
	protected function toString() {
		...
		if (isset($this->array[self::GOTO_PREV_X]))
			$_str .= $this->createHTMLLink($this->array[self::PAGES_PER_NAV].' stron(y) do tyłu', $this->array[self::GOTO_PREV_X][self::LINK], '<<').$sep;
		if (isset($this->array[self::GOTO_PREV]))
			$_str .= $this->createHTMLLink('Poprzednia strona', $this->array[self::GOTO_PREV][self::LINK], '<i class="fa fa-chevron-left"></i>').$sep;

		foreach ($this->array[self::PAGES] as $_page => $_pageLink){
			if (!$_pageLink)
				$_str .= '<li class="page-item active"><a class="page-link" href="#">'.$_page.' <span class="sr-only">(current)</span></a></li>';
			else
		...
		if (isset($this->array[self::GOTO_NEXT]))
			$_str .= $this->createHTMLLink('Następna strona', $this->array[self::GOTO_NEXT][self::LINK], '<i class="fa fa-chevron-right"></i>');
		if (isset($this->array[self::GOTO_NEXT_X]))
			$_str .= $sep.$this->createHTMLLink($this->array[self::PAGES_PER_NAV].' stron(y) do przodu', $this->array[self::GOTO_NEXT_X][self::LINK], '>>');
	...
	protected function createHTMLLink($title, $link, $text)
	{
		...
		/*if ($this->formatSQL($_GET['id'], 'int') == 0)
		{
			$row['id'] = 0;
			$row['nazwa'] = 'Wszystkie';
		} else $row = $db->query("SELECT id, nazwa FROM ".$this->prefix."_kategorie WHERE id=".$this->formatSQL($_GET['id'], 'int')." LIMIT 1")->fetch_array(MYSQLI_ASSOC);*/

		$getData = $_GET;
		$linkMain = ($_GET['op'] == 'items') ? ADMIN_FILE.'.php' : 'funcs.php';

		#parse_str($szukajLink, $getData);
		unset($getData['s']);

		$szukajLink = '&'.http_build_query($getData);

		if ($_GET['orderby']) $orderby = '&amp;orderby='.$_GET['orderby'];
		if ($_GET['end'] == 1) $end .= '&amp;end=1';
		if ($_GET['type'] == 'companies') $link .= '&amp;type=companies';
		$file = ($_GET['file'] === 'profile') ? 'profile' : 'list';

		return '<li class="page-item"><a class="page-link" title="'.$title.'" href="'.$classMain->mainConfig->siteurl.'/'.$linkMain.$link.$orderby.$szukajLink.$end.'#list">'.$text.'</a></li>';
	}
